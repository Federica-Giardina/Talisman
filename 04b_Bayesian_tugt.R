...
Combined_TUGT_tmnt_set <- read_excel("Combined_TUGT_tmnt_set.xlsx")

#aggregated
Combined_TUGT_tmnt_set %>% group_by(Castor.Participant.ID) %>% mutate(id = cur_group_id()) -> 
  TUGT_combined

TUGT_combined %>% arrange(id) %>% select (id, Treatment, Set, 
                                          pre_TUGT_time_avg, post_TUGT_time_avg)-> TUGT_combined

data <- list(n = 20, y_p = t(structure(c(TUGT_combined %>% filter(Treatment==5)%>% mutate(delta=post_TUGT_time_avg - pre_TUGT_time_avg)%>%pull(delta)),
                                       dim=c(2,20))), 
             y_t1 = t(structure(c(TUGT_combined %>%
                                    filter(Treatment==1)%>% mutate(delta=post_TUGT_time_avg - pre_TUGT_time_avg)%>%pull(delta)),
                                dim=c(2,20))),
             y_t2 = t(structure(c(TUGT_combined %>%
                                    filter(Treatment==2)%>% mutate(delta=post_TUGT_time_avg - pre_TUGT_time_avg)%>%pull(delta)),
                                dim=c(2,20))),
             y_t3 = t(structure(c(TUGT_combined %>%
                                    filter(Treatment==3)%>% mutate(delta=post_TUGT_time_avg - pre_TUGT_time_avg)%>%pull(delta)),
                                dim=c(2,20))),
             y_t4 = t(structure(c(TUGT_combined %>%
                                    filter(Treatment==4)%>% mutate(delta=post_TUGT_time_avg - pre_TUGT_time_avg)%>%pull(delta)),
                                dim=c(2,20))),
...
#splot(model.samples)
Bayes_TUGT<- summary(model.samples)
...
write.csv(combined_df, file = "Bayes_TUGT_summary.csv", row.names = FALSE)
...
write_xlsx(combined_df_rounded, "Bayes_TUGT_summary.xlsx")
