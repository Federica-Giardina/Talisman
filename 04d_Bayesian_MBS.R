...
Combined_sums_MiniBES_tmnt_set <- read_excel("Combined_sums_MiniBES_tmnt_set.xlsx")

#aggregated

Combined_sums_MiniBES_tmnt_set %>% group_by(Castor.Participant.ID) %>% mutate(id = cur_group_id()) -> 
  MiniBES_combined

MiniBES_combined %>% arrange(id) %>% select (id, Treatment, Set, 
                                           pre_MiniBES_sum, post_MiniBES_sum) -> MiniBES_combined

data <- list(n = 20, y_p = t(structure(c(MiniBES_combined %>% filter(Treatment==5)%>% mutate(delta=post_MiniBES_sum - pre_MiniBES_sum)%>%pull(delta)),
                                       dim=c(2,20))), 
             y_t1 = t(structure(c(MiniBES_combined %>%
                                    filter(Treatment==1)%>% mutate(delta=post_MiniBES_sum - pre_MiniBES_sum)%>%pull(delta)),
                                dim=c(2,20))),
             y_t2 = t(structure(c(MiniBES_combined %>%
                                    filter(Treatment==2)%>% mutate(delta=post_MiniBES_sum - pre_MiniBES_sum)%>%pull(delta)),
                                dim=c(2,20))),
             y_t3 = t(structure(c(MiniBES_combined %>%
                                    filter(Treatment==3)%>% mutate(delta=post_MiniBES_sum - pre_MiniBES_sum)%>%pull(delta)),
                                dim=c(2,20))),
             y_t4 = t(structure(c(MiniBES_combined %>%
                                    filter(Treatment==4)%>% mutate(delta=post_MiniBES_sum - pre_MiniBES_sum)%>%pull(delta)),
                                dim=c(2,20))),
...
#plot(model.samples)
Bayes_MBS <- summary(model.samples)



# Extract components from the summary object
stats_df <- as.data.frame(Bayes_MBS$statistics)
quant_df <- as.data.frame(Bayes_MBS$quantiles)
...
write.csv(combined_df, file = "Bayes_MPS_summary.csv", row.names = FALSE)
...
write_xlsx(combined_df_rounded, "Bayes_MPS_summary.xlsx")
