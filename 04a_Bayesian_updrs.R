...
setwd("C:/Users/feder/Dropbox/Documents/consulting/trials/N0f1/Talisman")


UPDRS<-read_excel("UPDRS.xlsx")

#aggregated

UPDRS %>% group_by(Castor.Participant.ID) %>% mutate(id = cur_group_id()) -> 
  UPDRS_combined

UPDRS_combined %>% arrange(id) -> UPDRS_combined

data <- list(n = 20, y_p = t(structure(c(UPDRS_combined %>% filter(Treatment==5)%>% mutate(delta=post_UPDRS_sum - pre_UPDRS_sum)%>%pull(delta)),
                                       dim=c(2,20))), 
             y_t1 = t(structure(c(UPDRS_combined %>%
                                    filter(Treatment==1)%>% mutate(delta=post_UPDRS_sum - pre_UPDRS_sum)%>%pull(delta)),
                                dim=c(2,20))),
             y_t2 = t(structure(c(UPDRS_combined %>%
                                    filter(Treatment==2)%>% mutate(delta=post_UPDRS_sum - pre_UPDRS_sum)%>%pull(delta)),
                                dim=c(2,20))),
             y_t3 = t(structure(c(UPDRS_combined %>%
                                    filter(Treatment==3)%>% mutate(delta=post_UPDRS_sum - pre_UPDRS_sum)%>%pull(delta)),
                                dim=c(2,20))),
             y_t4 = t(structure(c(UPDRS_combined %>%
                                    filter(Treatment==4)%>% mutate(delta=post_UPDRS_sum - pre_UPDRS_sum)%>%pull(delta)),
                                dim=c(2,20))),
...
#plot(model.samples)
Bayes_UPDRS <- summary(model.samples)


# Extract components from the summary object
stats_df <- as.data.frame(Bayes_UPDRS$statistics)
quant_df <- as.data.frame(Bayes_UPDRS$quantiles)
...
write.csv(combined_df, file = "Bayes_UPDRS_summary.csv", row.names = FALSE)
...
write_xlsx(combined_df_rounded, "Bayes_UPDRS_summary.xlsx")
